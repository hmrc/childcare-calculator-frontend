...
awk '/class/ {\
     print;\
     print "  def hasYourTaxCodeBeenAdjusted: Option[Boolean] = cacheMap.getEntry[Boolean](HasYourTaxCodeBeenAdjustedId.toString)";\
     print "";\
...
awk '/class/ {\
     ...
     print "    x => AnswerRow(\"hasYourTaxCodeBeenAdjusted.checkYourAnswersLabel\", if(x) \"site.yes\" else \"site.no\", true, routes.HasYourTaxCodeBeenAdjustedController.onPageLoad(CheckMode).url)"; print "  }";\
