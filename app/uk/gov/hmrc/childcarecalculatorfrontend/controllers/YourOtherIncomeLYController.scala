...
class YourOtherIncomeLYController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad(mode: Mode) = (getData andThen requireData) {
    implicit request =>
      ...
      Ok(yourOtherIncomeLY(appConfig, preparedForm, mode))
  }

  def onSubmit(mode: Mode) = (getData andThen requireData).async {
    implicit request =>
      BooleanForm("yourOtherIncomeLY.error").bindFromRequest().fold(
        (formWithErrors: Form[Boolean]) =>
          Future.successful(BadRequest(yourOtherIncomeLY(appConfig, formWithErrors, mode))),
        (value) =>
