...
import uk.gov.hmrc.childcarecalculatorfrontend.utils.ChildcareConstants.universalCreditErrorKey
...
class TaxOrUniversalCreditsController @Inject()(
  ...
  def onSubmit(mode: Mode): Action[AnyContent] = (getData andThen requireData).async {
    implicit request =>
      BooleanForm(universalCreditErrorKey).bindFromRequest().fold(
