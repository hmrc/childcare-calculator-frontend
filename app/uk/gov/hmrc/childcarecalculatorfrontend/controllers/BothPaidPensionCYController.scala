...
class BothPaidPensionCYController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad(mode: Mode) = (getData andThen requireData) {
    implicit request =>
      ...
      Ok(bothPaidPensionCY(appConfig, preparedForm, mode))
  }

  def onSubmit(mode: Mode) = (getData andThen requireData).async {
    implicit request =>
      BooleanForm("bothPaidPensionCY.error").bindFromRequest().fold(
        (formWithErrors: Form[Boolean]) =>
          Future.successful(BadRequest(bothPaidPensionCY(appConfig, formWithErrors, mode))),
        (value) =>
