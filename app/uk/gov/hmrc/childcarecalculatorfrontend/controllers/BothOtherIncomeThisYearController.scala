...
class BothOtherIncomeThisYearController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] = getData.andThen(requireData) { implicit request =>
    ...
    Ok(bothOtherIncomeThisYear(appConfig, preparedForm, taxYearInfo))
  }
    ...
    BooleanForm("bothOtherIncomeThisYear.error.notCompleted")
      ...
      .fold(
        (formWithErrors: Form[Boolean]) =>
          Future.successful(BadRequest(bothOtherIncomeThisYear(appConfig, formWithErrors, taxYearInfo))),
        value =>
          dataCacheConnector
