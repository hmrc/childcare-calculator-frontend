...
class BothOtherIncomeThisYearController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad(mode: Mode) = (getData andThen requireData) {
    implicit request =>
      ...
      Ok(bothOtherIncomeThisYear(appConfig, preparedForm, mode))
  }

  def onSubmit(mode: Mode) = (getData andThen requireData).async {
    implicit request =>
      BooleanForm("bothOtherIncomeThisYear.error").bindFromRequest().fold(
        (formWithErrors: Form[Boolean]) =>
          Future.successful(BadRequest(bothOtherIncomeThisYear(appConfig, formWithErrors, mode))),
        (value) =>
