...
class AboutYourChildController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad(mode: Mode) = (getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.aboutYourChild match {
        case None => AboutYourChildForm()
        case Some(value) => AboutYourChildForm().fill(value)
      }
      Ok(aboutYourChild(appConfig, preparedForm, mode))
  }
    ...
    implicit request =>
      AboutYourChildForm().bindFromRequest().fold(
        (formWithErrors: Form[AboutYourChild]) =>
          Future.successful(BadRequest(aboutYourChild(appConfig, formWithErrors, mode))),
        (value) =>
          dataCacheConnector.save[AboutYourChild](request.sessionId, AboutYourChildId.toString, value).map(cacheMap =>
