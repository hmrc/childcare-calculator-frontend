...
@Singleton
class DoYouOrPartnerGetBenefitsController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad(): Action[AnyContent] = (getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.doYouOrPartnerGetBenefits match {
        case None => DoYouOrPartnerGetBenefitsForm()
        case Some(value) => DoYouOrPartnerGetBenefitsForm().fill(value)
      }
  ...
  def onSubmit(): Action[AnyContent] = (getData andThen requireData).async {
    implicit request =>
      DoYouOrPartnerGetBenefitsForm().bindFromRequest().fold(
        (formWithErrors: Form[String]) =>
          Future.successful(BadRequest(doYouOrPartnerGetBenefits(formWithErrors))),
        value =>
          dataCacheConnector.save[String](request.sessionId, DoYouOrPartnerGetBenefitsId.toString, value).map(cacheMap =>
