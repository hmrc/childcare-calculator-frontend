...
class HasYourTaxCodeBeenAdjustedController @Inject()(
  ...
  def onPageLoad(mode: Mode) = (getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.hasYourTaxCodeBeenAdjusted match {
        case None => HasYourTaxCodeBeenAdjustedForm()
        case Some(value) => HasYourTaxCodeBeenAdjustedForm().fill(value)
      }
  ...
  def onSubmit(mode: Mode) = (getData andThen requireData).async {
    implicit request =>
      HasYourTaxCodeBeenAdjustedForm().bindFromRequest().fold(
        (formWithErrors: Form[String]) =>
          Future.successful(BadRequest(hasYourTaxCodeBeenAdjusted(appConfig, formWithErrors, mode))),
        (value) =>
          dataCacheConnector.save[String](request.sessionId, HasYourTaxCodeBeenAdjustedId.toString, value).map(cacheMap =>
