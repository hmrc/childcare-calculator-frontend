...
class PartnerPaidPensionPYController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad(mode: Mode) = (getData andThen requireData) {
    implicit request =>
      ...
      Ok(partnerPaidPensionPY(appConfig, preparedForm, mode))
  }

  def onSubmit(mode: Mode) = (getData andThen requireData).async {
    implicit request =>
      BooleanForm("partnerPaidPensionPY.error").bindFromRequest().fold(
        (formWithErrors: Form[Boolean]) =>
          Future.successful(BadRequest(partnerPaidPensionPY(appConfig, formWithErrors, mode))),
        (value) =>
