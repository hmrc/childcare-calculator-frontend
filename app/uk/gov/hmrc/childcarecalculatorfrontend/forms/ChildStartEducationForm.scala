...
object ChildStartEducationForm extends FormErrorHelper {
  ...
  def childStartEducationFormatter(errorKeyBlank: String, errorKeyDecimal: String, errorKeyNonNumeric: String) = new Formatter[Int] {
  ...
  def apply(errorKeyBlank: String = "error.required", errorKeyDecimal: String = "error.integer", errorKeyNonNumeric: String = "error.non_numeric"): Form[Int] =
    Form(single("value" -> of(childStartEducationFormatter(errorKeyBlank, errorKeyDecimal, errorKeyNonNumeric))))
}
