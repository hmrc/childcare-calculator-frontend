...
class FrontendAppConfig @Inject() (config: ServicesConfig, val configuration: Configuration) {
  ...
  lazy val maxFreeHoursAmount: String = if(allowMaxFreeHoursFromNineMonths) "30" else "15"
  lazy val allowFreeHoursFromNineMonths: Boolean = !LocalDate.now().isBefore(nineMonthRuleCutoff)
