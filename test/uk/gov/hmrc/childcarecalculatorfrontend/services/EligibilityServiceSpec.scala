...
class EligibilityServiceSpec extends SchemeSpec with MockitoSugar {
  ...
  "EligibilityService" must {
    ...
    "given a user input with location" in {
      ...
      eligibilityService.eligibility(answers) mustEqual schemeResults
//      connector.getEligibility(household) mustEqual schemeResults
    }
