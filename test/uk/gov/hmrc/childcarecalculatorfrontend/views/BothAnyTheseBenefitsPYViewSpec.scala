...
class BothAnyTheseBenefitsPYViewSpec extends YesNoViewBehaviours {
  ...
  def createView = () => bothAnyTheseBenefitsPY(frontendAppConfig, BooleanForm(), NormalMode)(fakeRequest, messages)

  def createViewUsingForm = (form: Form[Boolean]) => bothAnyTheseBenefitsPY(frontendAppConfig, form, NormalMode)(fakeRequest, messages)
