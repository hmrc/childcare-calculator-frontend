  ...
  "Result eligible view" must {
    ...
    "Contain results" when {
      "We have free hours value" in {
        val model = ResultsViewModel(freeHours = Some(15),location = Location.ENGLAND)
      ...
      "User is eligible for TC scheme" in {
        val modelWithLessThan1000 = ResultsViewModel(tc = Some(500),location = Location.ENGLAND)
        val modelWithMoreThan1000 = ResultsViewModel(tc = Some(1500),location = Location.ENGLAND)
     ...
     "User is eligible for TFC scheme" in {
        val modelWithLessThan1000 = ResultsViewModel(tfc = Some(600),location = Location.ENGLAND)
        val modelWithMoreThan1000 = ResultsViewModel(tfc = Some(1600),location = Location.ENGLAND)
      ...
      "User is eligible for ESC scheme" in {
        val modelWithLessThan1000 = ResultsViewModel(esc = Some(900),location = Location.ENGLAND)
        val modelWithMoreThan1000 = ResultsViewModel(tfc = Some(1900),location = Location.ENGLAND)
    ...
    "display correct no of free hours and text when user is eligible for free hours" when {
      "location is England and no of hours is 15" in {
        val model = ResultsViewModel(freeHours = Some(BigDecimal(15)), location = Location.ENGLAND)
        val view = asDocument(resultEligible(model, utils)(fakeRequest, messages))
      ...
      "location is England and no of hours is 30" in {
        val model = ResultsViewModel(freeHours = Some(BigDecimal(30)), location = Location.ENGLAND)
        val view = asDocument(resultEligible(model, utils)(fakeRequest, messages))
      ...
      "location is Wales" in {
        val model = ResultsViewModel(freeHours = Some(10), location =Location.WALES)
        val view = asDocument(resultEligible(model, utils)(fakeRequest, messages))
      ...
      "location is Scotland" in {
        val model = ResultsViewModel(freeHours = Some(16), location = Location.SCOTLAND)
        val view = asDocument(resultEligible(model, utils)(fakeRequest, messages))
      ...
      "location is NI" in {
        val model = ResultsViewModel(freeHours = Some(12.5), location = Location.NORTHERN_IRELAND)
        val view = asDocument(resultEligible(model, utils)(fakeRequest, messages))
