...
class PartnerAnyTheseBenefitsPYViewSpec extends NewYesNoViewBehaviours {

  override val form = BooleanForm()
  val taxYearInfo = new TaxYearInfo
  val view = application.injector.instanceOf[partnerAnyTheseBenefitsPY]
  ...
  "PartnerAnyTheseBenefitsPY view for non scottish users" must {

    val location: Location.Value = Location.ENGLAND
    behave like normalPage(createView(location:Location.Value), messageKeyPrefix, "li.income_support", "li.jobseekers_allowance",
    ...
    behave like yesNoPage(createViewUsingForm(location:Location.Value), messageKeyPrefix, routes.PartnerAnyTheseBenefitsPYController.onSubmit(NormalMode).url)
  ...
  "PartnerAnyTheseBenefitsPY view for scottish users" must {

    val location: Location.Value = Location.SCOTLAND
    behave like normalPage(createView(location:Location.Value), messageKeyPrefix, "li.income_support", "li.jobseekers_allowance",
      "li.employment_support", "li.pensions", "li.disability", "li.attendance", "li.independance", "li.scottishCarersAllowance")
