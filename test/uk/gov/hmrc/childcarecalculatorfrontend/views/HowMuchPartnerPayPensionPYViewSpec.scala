...
class HowMuchPartnerPayPensionPYViewSpec extends NewBigDecimalViewBehaviours {

  val view = application.injector.instanceOf[howMuchPartnerPayPensionPY]
  val messageKeyPrefix = "howMuchPartnerPayPensionPY"

  def createView = () => view(frontendAppConfig, HowMuchPartnerPayPensionPYForm(), NormalMode)(fakeRequest, messages)
  ...
  "HowMuchPartnerPayPensionPY view" must {
    ...
    behave like bigDecimalPage(
      createViewUsingForm,
      messageKeyPrefix,
      routes.HowMuchPartnerPayPensionPYController.onSubmit(NormalMode).url,
