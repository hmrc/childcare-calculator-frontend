...
class ResultViewSpec extends ViewBehaviours with MockitoSugar {
  ...
  def createView() = () => result(frontendAppConfig: FrontendAppConfig, ResultsViewModel(tc = Some(400),location = Location.ENGLAND, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true, livesWithPartner = true), List.empty, None, new Utils, false)(fakeRequest, messages)
  ...
  "Result view" must {
    ...
    "display more info about the schemes" in {
      ...
      val view = asDocument(result(frontendAppConfig, model,List(Map("title"->"test","link"->"test")) , None, new Utils, hideTC = false)(fakeRequest, messages))
  ...
  "Early results page" when {
    "rendered" must {
      ...
      "eligible for 15 free hours for England, gets uc and not eligible for other schemes and hideTC is true" in {
        val model = ResultsViewModel(freeHours = Some(15), location = Location.ENGLAND, isAnyoneInPaidEmployment = true, hasChildcareCosts = true,livesWithPartner = true,hasCostsWithApprovedProvider = true)
        val doc = asDocument(result(frontendAppConfig, model, List.empty, None, new Utils, hideTC = true)(fakeRequest, messages))
