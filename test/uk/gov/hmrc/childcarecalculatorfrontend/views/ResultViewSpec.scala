...
class ResultViewSpec extends ViewBehaviours with MockitoSugar {
  ...
  def createView() = () => resultView(frontendAppConfig: FrontendAppConfig, ResultsViewModel(tc = Some(400),location = Location.ENGLAND, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true, livesWithPartner = true), List.empty, None, new Utils, false)(fakeRequest, messages)
  ...
  "Result view" must {
    ...
    "contain two year old section" in {
      ...
      val view = asDocument(resultView(frontendAppConfig, model, List.empty, None, new Utils, hideTC = false)(fakeRequest, messages))
    ...
    "display more info about the schemes" in {
      ...
      val view = asDocument(resultView(frontendAppConfig, model,List(Map("title"->"test","link"->"test")) , None, new Utils, hideTC = false)(fakeRequest, messages))
  ...
  "Early results page" when {
    "rendered" must {
      "contain correct guidance when not eligible for location other than northern-ireland when don't have childcare cost and not eligible" in {
        ...
        val doc = asDocument(resultView(frontendAppConfig, model, List.empty, None, new Utils, hideTC = false)(fakeRequest, messages))
      ...
      "eligible for 15 free hours for England, gets uc and not eligible for other schemes and hideTC is true" in {
        val model = ResultsViewModel(freeHours = Some(15), location = Location.ENGLAND, isAnyoneInPaidEmployment = true, hasChildcareCosts = true,livesWithPartner = true,hasCostsWithApprovedProvider = true)
        val doc = asDocument(resultView(frontendAppConfig, model, List.empty, None, new Utils, hideTC = true)(fakeRequest, messages))
