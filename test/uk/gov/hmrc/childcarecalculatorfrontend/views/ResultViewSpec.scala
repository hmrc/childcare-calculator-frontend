...
class ResultViewSpec extends NewViewBehaviours with MockitoSugar {
  ...
  def createView(): () => HtmlFormat.Appendable = () => resultView(frontendAppConfig: FrontendAppConfig, ResultsViewModel(tc = Some(400), location = Location.ENGLAND, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true, livesWithPartner = true), new Utils, false)(fakeRequest, messages)
  ...
  "Result view" must {
    ...
    "contain two year old section" in {
      ...
      val view = asDocument(resultView(frontendAppConfig, model, new Utils, hideTC = false)(fakeRequest, messages))
  ...
  "Early results page" when {
    "rendered" must {
      "eligible for 22 free hours for scotland and not eligible for other schemes" in {
        ...
        val doc = asDocument(resultView(frontendAppConfig, model, new Utils, hideTC = false)(fakeRequest, messages))
