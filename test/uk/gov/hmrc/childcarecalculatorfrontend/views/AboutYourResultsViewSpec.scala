...
class AboutYourResultsViewSpec extends ViewBehaviours {
  ...
  def createView() = () => aboutYourResults(frontendAppConfig, ResultsViewModel(location = locationEngland, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true), List.empty, None)(fakeRequest, messages)
  ...
  "AboutYourResults view" must {
    ...
    "contain back to results link" in {
      ...
      val doc = asDocument(aboutYourResults(frontendAppConfig, ResultsViewModel(location = locationEngland, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true), List.empty, None)(fakeRequest, messages))
   ...
   "not display TFC contents" when {
    ...
    "not display guidance for 2 years old" when {
      "user does not have 2 years old child" in {
        val model = ResultsViewModel(freeHours = Some(15), tc = Some(200),location = locationEngland, childAgedTwo = false, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true)
    ...
    "display correct guidance" when {
      "users are not in England and are eligible for TFC" in {
        val modelWithWales = ResultsViewModel(tfc = Some(200), location = Location.WALES, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true)
        val modelWithScotland = ResultsViewModel(tfc = Some(200), location = Location.SCOTLAND, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true)
        val modelWithNI = ResultsViewModel(tfc = Some(200), location = Location.NORTHERN_IRELAND, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true)
