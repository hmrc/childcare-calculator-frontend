...
class PartnerEmploymentIncomeCYFormSpec extends FormSpec {
  ...
  "PartnerEmploymentIncomeCY Form" must {
    ...
    "bind zero" in {
      val form = PartnerEmploymentIncomeCYForm(errorKeyBlank, errorKeyInvalid).bind(Map("value" -> "0.0"))
    ...
    "bind positive decimal number" in {
      val form = PartnerEmploymentIncomeCYForm(errorKeyBlank, errorKeyInvalid).bind(Map("value" -> "10.80"))
