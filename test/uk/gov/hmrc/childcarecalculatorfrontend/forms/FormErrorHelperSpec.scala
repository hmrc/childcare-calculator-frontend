  ...
  "FormErrorHelper" when {
    "validateBothMaxIncomeEarnings" must {
      ...
      "return the form with error when partner answered max earnings question under 100000 but input was above 100000" in {
        ...
        val inputForm = new EmploymentIncomeCYForm(frontendAppConfig)
          .apply()
          .bind(Map("parentEmploymentIncomeCY" -> "8000", "partnerEmploymentIncomeCY" -> "100000"))

        val formWithError =
          inputForm.withError("partnerEmploymentIncomeCY", partnerEmploymentIncomeInvalidMaxEarningsErrorKey)

        formErrorHelper.validateBothMaxIncomeEarnings(maximumEarnings, maxIncome, inputForm) mustBe formWithError
      }
      ...
      "return the form with error when parent answered max earnings question under 100000 but input was above 100000" in {
        ...
        val inputForm = new EmploymentIncomeCYForm(frontendAppConfig)
          .apply()
          .bind(Map("parentEmploymentIncomeCY" -> "100000", "partnerEmploymentIncomeCY" -> "8000"))

        val formWithError =
          inputForm.withError("parentEmploymentIncomeCY", parentEmploymentIncomeInvalidMaxEarningsErrorKey)
      ...
      "return the form with error when both answered max earnings question under 100000 but input was above 100000" in {
        ...
        val inputForm = new EmploymentIncomeCYForm(frontendAppConfig)
          .apply()
          .bind(Map("parentEmploymentIncomeCY" -> "100000", "partnerEmploymentIncomeCY" -> "100000"))

        val formWithError = inputForm
          .withError("parentEmploymentIncomeCY", parentEmploymentIncomeBothInvalidMaxEarningsErrorKey)
      ...
      "return the same form without error when both answered max earnings question under 100000 and input was below 100000" in {
        ...
        val inputForm = new EmploymentIncomeCYForm(frontendAppConfig)
          .apply()
          .bind(Map("parentEmploymentIncomeCY" -> "8000", "partnerEmploymentIncomeCY" -> "8000"))

        formErrorHelper.validateBothMaxIncomeEarnings(maximumEarnings, maxIncome, inputForm) mustBe inputForm
      }
      ...
      "return the form when both answered max earnings question above 1000000 but input was above 1000000" in {
        ...
        val inputForm = new EmploymentIncomeCYForm(frontendAppConfig)
          .apply()
          .bind(Map("parentEmploymentIncomeCY" -> "1000000", "partnerEmploymentIncomeCY" -> "1000000"))
      ...
      "return the form with error when partner answered max earnings question under 1000000 but input was above 1000000" in {
        ...
        val inputForm = new EmploymentIncomeCYForm(frontendAppConfig)
          .apply()
          .bind(Map("parentEmploymentIncomeCY" -> "80000", "partnerEmploymentIncomeCY" -> "1000000"))
      ...
      "return the form with error when parent answered max earnings question under 1000000 but input was above 1000000" in {
        ...
        val inputForm = new EmploymentIncomeCYForm(frontendAppConfig)
          .apply()
          .bind(Map("parentEmploymentIncomeCY" -> "1000000", "partnerEmploymentIncomeCY" -> "100000"))
    ...
    "validateMaxIncomeEarnings" must {
      "return the original form if input value is good in case of Parent employment form" in {
        ...
        val maximumEarnings = Some(true)
        val inputForm =
          new ParentEmploymentIncomeCYForm(frontendAppConfig).apply().bind(Map(defaultFormValueField -> "80000"))

        formErrorHelper.validateMaxIncomeEarnings(maximumEarnings, maxIncome, "invalid key", inputForm) mustBe inputForm
      }
      ...
      "return the original form if input value is good in case of Partner employment form" in {
        ...
        val maximumEarnings = Some(true)
        val inputForm =
          new PartnerEmploymentIncomeCYForm(frontendAppConfig).apply().bind(Map(defaultFormValueField -> "80000"))
      ...
      "return the form with correct error if user has max earnings below 100,000 but input income value is more than 99,999.9 " in {
        ...
        val maximumEarnings = Some(false)
        val inputForm =
          new PartnerEmploymentIncomeCYForm(frontendAppConfig).apply().bind(Map(defaultFormValueField -> "100000"))

        val formWithError = inputForm.withError(defaultFormValueField, errorKeyInvalidMaxEarnings)

        formErrorHelper.validateMaxIncomeEarnings(
          maximumEarnings,
          maxIncome,
          errorKeyInvalidMaxEarnings,
          inputForm
        ) mustBe formWithError
      }
      ...
      "return the original form if user has max earnings over 100,000 and input income value is more than 99,999.9 and below 999,999.9 " in {
        ...
        val maximumEarnings = Some(true)
        val inputForm =
          new PartnerEmploymentIncomeCYForm(frontendAppConfig).apply().bind(Map(defaultFormValueField -> "102000"))
      ...
      "validate both Max Income Earnings form with validation error boundary check no partner income" in {
        ...
        val formWithError =
          inputForm.withError(parentEmpIncomeCYFormField, parentEmploymentIncomeInvalidMaxEarningsErrorKey)
