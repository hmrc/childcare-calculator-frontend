...
class ParentEmploymentIncomeCYFormSpec extends FormSpec {
  ...
  "ParentEmploymentIncomeCY Form" must {
    ...
    "bind zero" in {
      val form = ParentEmploymentIncomeCYForm(errorKeyBlank, errorKeyInvalid).bind(Map("value" -> "0.0"))
    ...
    "bind positive decimal number" in {
      val form = ParentEmploymentIncomeCYForm(errorKeyBlank, errorKeyInvalid).bind(Map("value" -> "10.80"))
