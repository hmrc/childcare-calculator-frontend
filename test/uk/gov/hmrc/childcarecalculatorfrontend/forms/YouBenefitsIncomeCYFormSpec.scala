...
class YouBenefitsIncomeCYFormSpec extends FormSpec {
  ...
  "YouBenefitsIncomeCY Form" must {
    ...
    "bind zero" in {
      val form = YouBenefitsIncomeCYForm(errorKeyBlank, errorKeyInvalid).bind(Map("value" -> "0.0"))
    ...
    "bind positive decimal number" in {
      val form = YouBenefitsIncomeCYForm(errorKeyBlank, errorKeyInvalid).bind(Map("value" -> "10.80"))
