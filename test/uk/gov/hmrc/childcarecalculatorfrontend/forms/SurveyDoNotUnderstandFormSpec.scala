...
class SurveyDoNotUnderstandFormSpec extends FormSpec {
  ...
  "SurveyDoNotUnderstand Form" must {
    ...
    "bind zero" in {
      val form = SurveyDoNotUnderstandForm(errorKeyBlank, errorKeyInvalid).bind(Map("value" -> "0.0"))
      form.get shouldBe 0.0
    }

    "bind positive numbers" in {
      val form = SurveyDoNotUnderstandForm(errorKeyBlank, errorKeyInvalid).bind(Map("value" -> "1.0"))
      form.get shouldBe 1.0
    }

    "bind positive decimal number" in {
      val form = SurveyDoNotUnderstandForm(errorKeyBlank, errorKeyInvalid).bind(Map("value" -> "10.80"))
      form.get shouldBe 10.80
    }

    "fail to bind negative numbers" in {
      val expectedError = error("value", errorKeyInvalid)
      checkForError(SurveyDoNotUnderstandForm(errorKeyBlank, errorKeyInvalid), Map("value" -> "-1"), expectedError)
    }

    "fail to bind non-numerics" in {
      val expectedError = error("value", errorKeyInvalid)
      checkForError(SurveyDoNotUnderstandForm(errorKeyBlank, errorKeyInvalid), Map("value" -> "not a number"), expectedError)
    }
