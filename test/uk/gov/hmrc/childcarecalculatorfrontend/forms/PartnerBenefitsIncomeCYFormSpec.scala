...
class PartnerBenefitsIncomeCYFormSpec extends FormSpec {
  ...
  "PartnerBenefitsIncomeCY Form" must {
    ...
    "fail to bind numbers above the threshold of 9999.99" in {
      ...
      checkForError(PartnerBenefitsIncomeCYForm(errorKeyBlank, errorKeyInvalid), Map("value" -> "10000"), expectedError)
    }

    "fail to bind numbers below the threshold of 1" in {
      val expectedError = error("value", errorKeyInvalid)
      checkForError(PartnerBenefitsIncomeCYForm(errorKeyBlank, errorKeyInvalid), Map("value" -> "0.9"), expectedError)
    }
