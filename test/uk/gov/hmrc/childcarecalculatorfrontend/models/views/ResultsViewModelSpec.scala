  ...
  "ResultViewModel" must {
    ...
    "display information about your two year old" when {
      "user does not live in Northern Ireland, has a two year old and either has a three year old or is eligible to any scheme" in {
        val model = ResultsViewModel(tfc= Some(30), location = Location.SCOTLAND, hasChildcareCosts = true, hasCostsWithApprovedProvider = true, isAnyoneInPaidEmployment = true, livesWithPartner = true, childrenAgeGroups = Set(TwoYears, FourYears))
