...
class ParentEmploymentIncomePYControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new ParentEmploymentIncomePYController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute),
      dataRetrievalAction, new DataRequiredActionImpl, new ParentEmploymentIncomePYForm(frontendAppConfig))

  def viewAsString(form: Form[BigDecimal] = employmentIncomeForm) = parentEmploymentIncomePY(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
