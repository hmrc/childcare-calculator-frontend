...
import uk.gov.hmrc.childcarecalculatorfrontend.identifiers.{BothAnyTheseBenefitsPYId, LocationId}
...
class BothAnyTheseBenefitsPYControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getDataWithLocationSet) =
    new BothAnyTheseBenefitsPYController(frontendAppConfig, mcc, FakeDataCacheService, new FakeNavigator(desiredRoute = onwardRoute),
  ...
  "BothAnyTheseBenefitsPY Controller" must {
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val boundForm = BooleanForm(bothAnyTheseBenefitsPYErrorKey).bind(Map("value" -> "invalid value"))
