...
class WhoGotStatutoryPayControllerSpec extends ControllerSpecBase {
  ...
  def viewAsString(form: Form[String] = WhoGotStatutoryPayForm()) = whoGotStatutoryPay(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "WhoGotStatutoryPay Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustBe viewAsString(WhoGotStatutoryPayForm().fill(WhoGotStatutoryPayForm.options.head.value))
    }
