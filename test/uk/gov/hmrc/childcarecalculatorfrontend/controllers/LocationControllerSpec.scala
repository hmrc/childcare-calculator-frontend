...
class LocationControllerSpec extends UnitSpec with FakeCCApplication with CCRoutes{

  val sut = new LocationController(applicationMessagesApi)

  s"${location} url" should {

    "be available" when {

      "GET request is made" in {
        val req = FakeRequest(GET, location).withSession(validSession)
  ...
  "LocationController" should {
    ...
    "load template successfully for the first time" when {
