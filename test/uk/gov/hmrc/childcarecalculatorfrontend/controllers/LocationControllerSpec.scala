...
class LocationControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new LocationController(frontendAppConfig, mcc, FakeDataCacheService, new FakeNavigator(desiredRoute = onwardRoute),
      dataRetrievalAction, new DataRequiredAction, view)

  def viewAsString(form: Form[_] = LocationForm()) = view(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "Location Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", LocationForm.options.head.value)).withMethod("POST")
