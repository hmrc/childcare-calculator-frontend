...
class BothOtherIncomeThisYearControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new BothOtherIncomeThisYearController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute),
      dataRetrievalAction, new DataRequiredActionImpl)

  def viewAsString(form: Form[Boolean] = BooleanForm()) = bothOtherIncomeThisYear(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "BothOtherIncomeThisYear Controller" must {
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val boundForm = BooleanForm("bothOtherIncomeThisYear.error").bind(Map("value" -> "invalid value"))
