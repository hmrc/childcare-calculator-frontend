...
class PartnerPaidWorkPYControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new PartnerPaidWorkPYController(frontendAppConfig, mcc, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute),
      dataRetrievalAction, new DataRequiredAction, taxYearInfo)

  def viewAsString(form: Form[Boolean] = BooleanForm()) = partnerPaidWorkPY(frontendAppConfig, form, NormalMode, taxYearInfo)(fakeRequest, messages).toString
  ...
  "PartnerPaidWorkPY Controller" must {
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val boundForm = BooleanForm(partnerPaidWorkPYErrorKey).bind(Map("value" -> "invalid value"))
