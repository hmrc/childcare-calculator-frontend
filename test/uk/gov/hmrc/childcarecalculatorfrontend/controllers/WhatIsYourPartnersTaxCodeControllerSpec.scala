...
class WhatIsYourPartnersTaxCodeControllerSpec extends ControllerSpecBase {
  ...
  def viewAsString(form: Form[Int] = WhatIsYourPartnersTaxCodeForm()) = whatIsYourPartnersTaxCode(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "WhatIsYourPartnersTaxCode Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(WhatIsYourPartnersTaxCodeId.toString -> JsNumber(testNumber))
      val getRelevantData = new FakeDataRetrievalAction(Some(CacheMap(cacheMapId, validData)))

      val result = controller(getRelevantData).onPageLoad(NormalMode)(fakeRequest)

      contentAsString(result) mustBe viewAsString(WhatIsYourPartnersTaxCodeForm().fill(testNumber))
    }
