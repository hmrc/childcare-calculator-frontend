...
class DoYouGetBenefitsControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap): DoYouGetBenefitsController =
    new DoYouGetBenefitsController(frontendAppConfig, mcc, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute),
      dataRetrievalAction, new DataRequiredAction, view)

  def viewAsString(form: Form[Boolean] = BooleanForm()): String = view(form)(fakeRequest, messages).toString

  "GET /you-get-benefits" must {
    ...
    "populate the view correctly when the question has previously been answered" in {
      val validData = Map(DoYouGetBenefitsId.toString -> JsBoolean(true))
  ...
  "POST /you-get-benefits" must {
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val boundForm = BooleanForm(doYouGetBenefitsErrorKey).bind(Map("value" -> "invalid value"))
