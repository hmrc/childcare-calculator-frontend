...
class DoYouGetDisabilityBenefitsControllerSpec extends ControllerSpecBase {

  val onwardRoute: Call = controllers.routes.YourAgeController.onPageLoad()
  val view: doYouGetDisabilityBenefits = application.injector.instanceOf[doYouGetDisabilityBenefits]

  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap): DoYouGetDisabilityBenefitsController =
    new DoYouGetDisabilityBenefitsController(mcc, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute),
      dataRetrievalAction, new DataRequiredAction, view)

  def viewAsString(form: Form[Boolean] = BooleanForm()): String = view(form)(fakeRequest, messages).toString

  "GET /you-get-disability-benefits" must {
  ...
  "populate the view correctly when the question has previously been answered" in {
    val validData = Map(DoYouGetDisabilityBenefitsId.toString -> JsBoolean(true))
  ...
  "POST /you-get-disability-benefits" must {
  ...
  "return a Bad Request and errors when invalid data is submitted" in {
    ...
    val boundForm = BooleanForm(doYouGetDisabilityBenefitsErrorKey).bind(Map("value" -> "invalid value"))
