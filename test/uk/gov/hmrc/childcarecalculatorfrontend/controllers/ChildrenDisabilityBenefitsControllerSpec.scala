...
class ChildrenDisabilityBenefitsControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new ChildrenDisabilityBenefitsController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute),
      dataRetrievalAction, new DataRequiredActionImpl)

  def viewAsString(form: Form[Boolean] = BooleanForm()) = childrenDisabilityBenefits(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "ChildrenDisabilityBenefits Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(ChildrenDisabilityBenefitsId.toString -> JsBoolean(true))
