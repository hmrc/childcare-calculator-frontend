...
class WhosHadBenefitsPYControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new WhosHadBenefitsPYController(frontendAppConfig, mcc, FakeDataCacheService, new FakeNavigator(desiredRoute = onwardRoute),
  ...
  "WhosHadBenefitsPY Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      ...
      contentAsString(result) mustBe viewAsString(WhosHadBenefitsPYForm().fill(YouPartnerBothEnum.YOU))
    }

    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("value", WhosHadBenefitsPYForm.options.head.value)).withMethod("POST")
