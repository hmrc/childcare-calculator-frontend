...
class BothGetSameIncomePreviousYearControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new BothGetSameIncomePreviousYearController(frontendAppConfig, mcc, FakeDataCacheService, new FakeNavigator(desiredRoute = onwardRoute),
  ...
  "BothGetSameIncomePreviousYear Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Map(BothGetSameIncomePreviousYearId.toString -> JsBoolean(true))
    ...
    "return a Bad Request and errors when invalid data is submitted" in {
      ...
      val boundForm = BooleanForm("bothGetSameIncomePreviousYear.error.notCompleted").bind(Map("value" -> "invalid value"))
