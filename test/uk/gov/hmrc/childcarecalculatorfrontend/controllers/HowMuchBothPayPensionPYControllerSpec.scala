...
import uk.gov.hmrc.childcarecalculatorfrontend.models.{HowMuchBothPayPensionPY, NormalMode}
...
class HowMuchBothPayPensionPYControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new HowMuchBothPayPensionPYController(frontendAppConfig, mcc, FakeDataCacheService, new FakeNavigator(desiredRoute = onwardRoute),
      dataRetrievalAction, new DataRequiredAction, view)

  def viewAsString(form: Form[HowMuchBothPayPensionPY] = HowMuchBothPayPensionPYForm()) = view(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "HowMuchBothPayPensionPY Controller" must {
    ...
    "redirect to the next page when valid data is submitted" in {
      val postRequest = fakeRequest.withFormUrlEncodedBody(("howMuchYouPayPensionPY", "1"), ("howMuchPartnerPayPensionPY", "2")).withMethod("POST")
