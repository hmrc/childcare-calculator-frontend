...
class NoOfChildrenControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyCacheMap) =
    new NoOfChildrenController(frontendAppConfig, mcc, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute),
      dataRetrievalAction, new DataRequiredAction, new NoOfChildrenForm(frontendAppConfig), view)

  def viewAsString(form: Form[Int] = NoOfChildrenForm) = view(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
